...
export function SavingsTrendChart({ data }: { data: { month: string; value: number }[] }) {
  return (
    <ChartCard title="Évolution de l'épargne" hint="6 derniers mois">
      <ResponsiveContainer width="100%" height={180}>
        <AreaChart data={data} margin={{ left: -18, right: 8, top: 8 }}>
          <defs>
            <linearGradient id="savingsFill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="hsl(var(--brand-gold))" stopOpacity={0.28} />
          ...
          />
          <Area type="monotone" dataKey="value" stroke="hsl(var(--brand-gold))" strokeWidth={2} fill="url(#savingsFill)" dot={false} isAnimationActive />
        </AreaChart>
      </ResponsiveContainer>
    </ChartCard>
  );
